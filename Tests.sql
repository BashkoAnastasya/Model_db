CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY test IS
  ...
  BEGIN
    ...
    DELETE FROM t_price_ware t
     WHERE t.id_ware in (1)
       AND t.dt_end > SYSDATE;
  ...
  FUNCTION f_test_supply_str_summa_nds(p_id_supply     t_supply.id_supply%TYPE,
                                       ...
                                       p_price         t_supply_str.price%TYPE)
    RETURN INTEGER IS
    v_result INTEGER;
  ...
  BEGIN
    ...
    IF NVL(v_summa, 0) <> p_price * p_qty OR
       NVL(v_nds, 0) <> ROUND(p_qty * p_price * 18 / 118, 2) THEN
      v_result := 1;
    END IF;
    SELECT summa, nds
      INTO v_summa, v_nds
    ...
    IF NVL(v_summa, 0) <> p_price * p_qty OR
       NVL(v_nds, 0) <> ROUND(p_price * p_qty * 18 / 118, 2) THEN
      v_result := 1;
    END IF;
    RETURN v_result;
  END;
  ...
  --TASK_4.2
  FUNCTION f_test_change_qty(p_id_sale_str t_sale_str.id_sale_str%TYPE,
                             p_qty         t_sale_str.qty%TYPE)
    RETURN INTEGER IS
    v_result INTEGER;
    v_summa  t_sale_str.summa%TYPE;
  ...
  --TASK_4.3
  FUNCTION f_test_change_discount(p_id_sale  t_sale.id_sale%TYPE,
                                  p_discount t_sale.discount%TYPE)
    RETURN INTEGER IS
    v_result    INTEGER;
    v_summa     t_sale_str.summa%TYPE;
  ...
  FUNCTION f_test_add_price(p_id_sale_str t_sale_str.id_sale_str%TYPE,
                            p_id_ware     t_price_ware.id_ware%TYPE,
                            p_price       t_price_ware.price%TYPE)
    RETURN INTEGER IS
    v_result  INTEGER;
    v_price   t_price_ware.price%TYPE;
  ...
  FUNCTION f_test_t_rest_sale(p_id_sale  t_sale.id_sale%TYPE,
                              p_state_id t_sale.id_state%TYPE) RETURN INTEGER IS
    v_result INTEGER;
    TYPE Rest_T IS TABLE OF VARCHAR2(5000);
    TV Rest_T;
    TR Rest_T;
  BEGIN
    ...
    SELECT *
      ...
      FROM (SELECT t.id_ware || ' ' || t.qty
              FROM t_rest t
             WHERE t.id_ware IN (SELECT t2.id_ware
                                   FROM t_sale_str t2
  ...
  BEGIN
    add_test_data;
    dbms_output.put_line('TASK_1.1 ' ||
                         f_test_supply_str_summa_nds(101,
                                                     'Test-1',
                                                     DATE '2018-09-01',
                                                     1,
                                                     101,
    ...
    dbms_output.put_line('TASK_2.1 ' ||
                         f_test_client_discount_nv(46,
                                                   1,
                                                   'Ivanov SS',
                                                   'Ivan',
                                                   'N',
    ...
    dbms_output.put_line('TASK_2.2 ' ||
                         f_test_client_discount_nv(46,
                                                   ...
                                                   DATE '2018-08-08',
                                                   21));
    dbms_output.put_line('TASK_3. ' || f_test_price(99,
                                                    DATE '2018-09-01',
                                                    10,
                                                    DATE '2018-09-01',
                                                    DATE '2018-09-29',
    ...
    dbms_output.put_line('TASK_5. ' || f_test_add_price(95, 1, 16));
